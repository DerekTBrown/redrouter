...
RedRouter.create = function(opts){
  ...
  // Load Proxy Agents
    ...
    HTTPSServer = require('https').createServer(_this.ssl).listen(opts.port, function(){
        redrouter.log('info','listening on https port '+ opts.port)
    });
  /**
    ...
    this.getHTTPS = function(port){
      if(typeof HTTPSServer[port] === "undefined" || HTTPSServer[port] === null){
        HTTPSServer[port] = require('https').createServer(_this.ssl).listen(port, function(){
            redrouter.log('info','listening for wetty on https port ' + port);
        });
      }
      else{
  ...
  **/
    this.destroy = function(){
      ...
      // Destroy HTTP Servers
      for each (server in HTTPServer){
        server.close();
      }
      for each (server in HTTPSServer){
        server.close();
      }
